...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'Region'
        db.create_table('django_geonames_region', (
        ...
        db.send_create_signal('django_geonames', ['Region'])

        # Adding unique constraint on 'Region', fields ['country', 'name']
        db.create_unique('django_geonames_region', ['country_id', 'name'])

        # Adding field 'City.region'
        db.add_column('django_geonames_city', 'region',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['django_geonames.Region'], null=True),
                      keep_default=False)

        # Adding unique constraint on 'City', fields ['region', 'name']
        db.create_unique('django_geonames_city', ['region_id', 'name'])


    def backwards(self, orm):
        # Removing unique constraint on 'City', fields ['region', 'name']
        db.delete_unique('django_geonames_city', ['region_id', 'name'])

        # Removing unique constraint on 'Region', fields ['country', 'name']
        db.delete_unique('django_geonames_region', ['country_id', 'name'])

        # Deleting model 'Region'
        db.delete_table('django_geonames_region')

        # Deleting field 'City.region'
        db.delete_column('django_geonames_city', 'region_id')
