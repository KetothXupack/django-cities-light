...
class Migration(DataMigration):
    def _get_country(self, code2):
        ...
        if code2 not in self._country_codes.keys():
            self._country_codes[code2] = self.orm['django_geonames.Country'].objects.get(code2=code2)
    ...
    def _get_region(self, country_code2, region_id):
        ...
        if region_id not in self._region_codes[country.code2]:
            self._region_codes[country.code2][region_id] = self.orm['django_geonames.Region'].objects.get(
    ...
    def forwards(self, orm):
        ...
        if not orm['django_geonames.City'].objects.count():
            return

        self.regions = {}
        for country in orm['django_geonames.Country'].objects.all():
        ...
        for url in REGION_SOURCES + CITY_SOURCES:
            ...
            if url in REGION_SOURCES:
                ...
                for items in geonames.parse():
                    ...
                    try:
                        city = orm['django_geonames.City'].objects.get(**kwargs)
                    except orm['django_geonames.City'].DoesNotExist:
                        continue
